...
class DQN_Learner(QLearn):
    def __init__(self, epsilon, alpha, gamma, reward_function, deepQNetwork):
    ...
    def choose_action_egreedy(self, state, return_q=False):
        qVals = self.network.getQValues(state)
...
class MountainCarConfig(MORLEnvironment):
    def __init__(self, learner=None, num_bins=1000):
        ...
        super(MountainCarConfig, self).__init__(learner_klass=learn, n_learners=1)
...
def Run_Example():
    ...
    learner = DQN_Learner(1, learningRate, discountFactor, lambda x: x[1], deepQNetwork=deepQ)
